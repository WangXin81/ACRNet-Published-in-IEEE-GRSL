...
from models.FPN_Resnet50 import *
# from models.AMSAF import *
# from models.RFA_resnet import resnet50
from models.CGC.imagenet import cgc_resnet50
import config
import json
import matplotlib.pyplot as plt
# import main
...
def plot_confusion_matrix(cm, labels):
    ...
    for x_val, y_val in zip(x.flatten(), y.flatten()):
        ...
        if (intFlag):
            c = cm[y_val][x_val]
            plt.text(x_val, y_val, "%d" % (c,), color='white', fontsize=12, va='center', ha='center')
            ...
            if (c > 0.001):
                ...
                plt.text(x_val, y_val, "%d" % (0,), color='black', fontsize=12, va='center', ha='center')
    if (intFlag):
    ...
    plt.xlabel('Predict Classes', fontsize=14)
    plt.savefig('confusion_matrix_AID111222.png', dpi=400, bbox_inches='tight')
    plt.show()
...
attack_types = ['agricultural','airplane','baseballdiamond','beach','buildings','chaparral','denseresidential','forest','freeway','golfcourse','harbor','intersection','mediumresidential','mobilehomepark','overpass','parkinglot','river','runway','sparseresidential','storagetanks','tenniscourt']
# attack_types = ['Airport','BareLand','BaseballField','Beach','Bridge','Center','Church','Commercial','DenseResidential','Desert','Farmland','Forest','Industrial','Meadow','MediumResidential','Mountain','Park','Parking','Playground','Pond','Port','RailwayStation','Resort','River','School','SparseResidential','Square','Stadium','StorageTanks','Viaduct']
